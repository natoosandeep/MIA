...
function _handleLayerDrag( $canvas, data, eventType ) {
	...
	if ( eventType === 'mousemove' || eventType === 'touchmove' ) {
		 ...
		 * @Author:bharat
		 * @Version: 1.0
		 * EDIT GATEWAY ON DROP
		 */
		//console.log(data.layers[0].gatewayId);
		//console.log(data.layers[0].uniqueId);
		var canvas = document.getElementById('imageCanvasForGatewaySetup');
		var ctx=canvas.getContext("2d");
		var rect = canvas.getBoundingClientRect(), // abs. size of element
        scaleX = canvas.width / rect.width,    // relationship bitmap vs. element for X
        scaleY = canvas.height / rect.height;
    	var x =(data.event.event.clientX - rect.left) * scaleX;
    	var y = (data.event.event.clientY - rect.top) * scaleY;
    	var top = x;
		var left = y;

		//ctx.clearRect( 0, 0, canvas.width, canvas.height );
		//CALL FUNCTION BY PASSING PARAMETER TO UPDATE COORDINATE POSITION
		for(var i=0; i<data.layers.length; i++){
			if(data.lastIntersected.uniqueId === data.layers[i].uniqueId){
				angular.element(document.getElementById('locationCtrl')).scope().editGatewayOnMap(data.layers[i].gatewayId,data.layers[i].uniqueId,top+':'+left);
			}
		}
			...
			if ( params.width === NULL || params.height === NULL ) {
		        ...
		        reader.onload = function(event){
		            ...
		            img.onload = function(){
		            	$canvases.width = 670;
		            	$canvases.height = 400;
		                ctx.drawImage(img,0,0,670,400);
		            }
		            img.src = sourceImageFromController;
		        }
		        reader.readAsDataURL(convertS3ImageToBlobDataForGatewaySetup);*/
